...
import { useScripts, type Page } from '@/hooks/use-scripts.tsx';
...
export default function PagePage({ params: rawParams }: { params: { id: string } }) {
    const params = React.use(rawParams);
    const router = useRouter();
    const { id: pageId } = params;
    const { pages, updatePage } = useScripts();
  ...
  const handleThemeChange = (newThemeId: ThemeId) => {
    if (page) {
        updatePage(page.id, { themeId: newThemeId });
    }
  }
  ...
  const currentThemeId = page?.themeId || theme;
  ...
  return (
    <div className="pb-12">
      ...
      <div className="px-4 sm:px-6 pt-8">
        ...
        />
        <div className="mt-8">
            <BlockEditor onUpdate={handleDescriptionChange} initialBlocks={page.description} />
        </div>
      </div>
    </div>
  );
