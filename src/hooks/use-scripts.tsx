...
interface ScriptContextType {
  ...
  findOrCreateConversation: (otherUserId: string) => Conversation;
...
export function ScriptProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const loadState = async () => {
      try {
        const savedState = await getFromStorage<AppState>('appGlobalData') || { 
          scripts: [], 
          journalEntries: [], 
          places: [], 
          pages: [], 
        ...
        if (user.id && user.id !== 'default-user') {
          const hasApeiron = savedState.conversations.some(c => c.participantIds.includes('apeiron-user') && c.participantIds.includes(user.id));
          if (!hasApeiron) {
  ...
  const debouncedSave = useDebouncedCallback(async (stateToSave: AppState) => {
    await saveToStorage('appGlobalData', stateToSave);
  }, 1000);
  ...
  }, [appState, isMounted, debouncedSave]);
  ...
  const setState = useCallback((updater: (prevState: AppState) => AppState, immediate = false) => {
    setUndoableState(current => {
      ...
      if (immediate) {
        saveToStorage('appGlobalData', newState);
      }
  ...
  const findOrCreateConversation = (otherUserId: string): Conversation => {
    let conversation: Conversation | undefined;
    setState(prev => {
        const existingConvo = prev.conversations.find(c => 
        ...
        return { ...prev, conversations: [...prev.conversations, newConversation] };
    }, true);
    return conversation!;
  }
