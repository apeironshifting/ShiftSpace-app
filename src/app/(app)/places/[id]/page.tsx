...
export default function PlacePage({ params: rawParams }: { params: { id: string } }) {
    const params = React.use(rawParams);
    const router = useRouter();
    const { id: placeId } = params;
    const { places, updatePlace } = useScripts();
  ...
  const handleThemeChange = (newThemeId: ThemeId) => {
    if (place) {
        updatePlace(place.id, { themeId: newThemeId });
    }
  }
