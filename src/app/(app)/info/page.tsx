...
export default function InfoPage() {
  ...
  useEffect(() => {
    ...
    const loadInfo = async () => {
      try {
        const savedSections = await getFromStorage<InfoSectionType[]>('infoSections');
        setSections(savedSections || []);
      } catch (error) {
        console.error("Failed to load info sections from IndexedDB", error);
      }
    };
    loadInfo();
  }, []);
  ...
  useEffect(() => {
    if (isMounted) {
      saveToStorage('infoSections', sections);
    }
  }, [sections, isMounted]);
